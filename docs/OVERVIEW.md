...
- **Version / Platform:** Extension v1.0.0.0 targeting Business Central 26.0+ (runtime 15.0) with object range 72930450-72930499.
...
| --- | --- | --- |
| Event Intake | Codeunit 72930451 `Event Handlers_TSA_TSL` | **Automatic** subscribers on the global database triggers (active in every session). Each event is gated by `Session Manager.ShouldCapture()` so only the recorded user's changes pass, then enforces capture guardrails (table filters, throttling, change thresholds) and serializes record snapshots. |
| Filtering & Setup | Codeunit 72930452 + Tables 72930451/72930452 | Centralized filter policy (table allow/deny, field projection, threshold checks, per-second limits) backed by `Setup_TSA_TSL` and `Table Filter` data. |
| Context Enrichment | Codeunit 72930453 `Context Manager` | Augments each change with user/session/company metadata, client type, rolling transaction IDs, real AL call stack (`SessionInformation.Callstack()`, BC 26+), and a derived trigger source label (Database Insert/Modify/Delete/Rename). |
| Session Orchestration | Codeunit 72930450 `Session Manager` | Starts/stops runs, persists recording state (active flag, run id, recorded user) in `Recording State_TSA_TSL` (table 72930457) so every session can read it, writes captures to the persisted `Change Buffer` (table 72930450), exposes live stats, and launches downstream pages. |
| Analytics | Codeunit 72930454 `Analysis Engine` + table 72930454 | Generates impact, performance, pattern/burst, relationship, and temporary-vs-real insights stored in the `Analysis Buffer`. |
| UI & Visualization | Pages 72930450-72930459 (+ helper pages) | Card/List pages for recording control, live stats, results, advanced analysis, and context/transaction/table views. |
...
3. **Field Filtering (optional, per table):**
   ...
   - On the Table Filters page, use the **Select Fields** action to tick the fields to capture for the current row (checkbox list via `Field Selection_TSA_TSL`, page 72930465, backed by the **persisted** table `Field Select Buffer_TSA_TSL`, 72930455, keyed by Table ID + Field No.). Edits save immediately and are retained across sessions; deleting a Table Filter row cascades to delete its stored field selections (unless another row still references the same Table ID).
...
- **Causes:**
  1. Context Manager not initialized → ensure codeunit 72930453 is SingleInstance and not cleared.
...
| --- | --- | --- | --- |
| `Change Type_TSA_TSL` | 72930450 | Insert, Modify, Delete, Rename, **Error** | Classifies the captured entry. `Error` (value 4) marks an entry sourced from an `Error Message` (table 700) insert, whose `Call Stack` holds the error-origin stack. Marked `Extensible = false`. |
| `Capture Scope_TSA_TSL` | 72930453 | All Tables, Only Selected Tables, All Except Selected Tables | Single Setup control deciding how the Table Filters list is interpreted (capture all / whitelist / blacklist). |
| `Analysis Type_TSA_TSL` | 72930451 | Impact Analysis, Performance Metric, Pattern Detection, Relationship Mapping | Categorizes analysis buffer entries. |
| `Severity_TSA_TSL` | 72930452 | Info, Warning, Critical | Severity classification for analysis findings. |

### Table Fields Deep Dive
**Change Buffer_TSA_TSL (72930450)**
...
**Setup_TSA_TSL (72930451)**
- **Singleton:** Primary Key = '' (empty string)
- **Table Filtering:** Table Capture Scope (enum `Capture Scope_TSA_TSL`: All Tables / Only Selected Tables / All Except Selected Tables; defaults to All Tables)
...
**Recording State_TSA_TSL (72930457)**
...
**Table Filter_TSA_TSL (72930452)**
- **Fields:** Entry No., Table ID (lookup to AllObjWithCaption), Table Name (auto-populated), Enabled (Boolean). Table membership is defined by a row's presence + Enabled; the Setup **Capture Scope** decides whether the list is a whitelist or blacklist. Per-field capture selections live in the separate persisted table `Field Select Buffer_TSA_TSL` (72930455), and are cascade-deleted via this table's OnDelete trigger.
- **Trigger Behavior:** OnInsert/OnModify automatically updates Table Name from metadata

**Live Stats_TSA_TSL (72930453)**
...
**Analysis Buffer_TSA_TSL (72930454)**
...
| Setup_TSA_TSL | 72930454 | Card | Table Filters | Opens Table Filters (72930455); edits Setup singleton |
| Table Filters_TSA_TSL | 72930455 | List | Add Common System Tables | Manages Table Filter (72930452) records; pre-populates exclusion list |
| Context Details_TSA_TSL | 72930456 | Card | Copy Call Stack, Export Context | Displays user/session/transaction/call stack for selected change |
| Transactions_TSA_TSL | 72930457 | List | View Transaction Changes | Groups changes by Transaction ID, drills into filtered Results |
| Advanced Analysis_TSA_TSL | 72930458 | List | Refresh Analysis, View Details, Export Analysis, Show Recommendations | Runs Analysis Engine, filters results, shows recommendations dialog |
| Live Stats_TSA_TSL | 72930459 | Card | Refresh, Detailed Analysis, View All Results | Auto-refresh stats, opens Advanced Analysis or Results from active session |
...
| --- | --- | --- | --- |
| `Change Entry API_TSA_TSL` (Page) | 72930463 | `changeEntries` | One row per captured change. Surfaces all context fields plus the `oldData`, `newData`, and `callStack` BLOBs decoded to text. Supports OData `$filter` (e.g. by `runId`, `tableId`, `changeType`). |
| `Recording Run API_TSA_TSL` (Query) | 72930450 | `recordingRuns` | Summary grouped by `runId` with `changeCount`, `firstChange`, `lastChange` — lets a client discover sessions before drilling into entries. |
...
- **User gating**: Every On* handler calls `Session Manager.ShouldCapture()`, which returns true only when a recording is active and `UserSecurityId()` matches the recorded user stored in `Recording State_TSA_TSL`. The state is cached per session for ~1 second to keep the always-on path cheap.
- **SingleInstance = true**: All manager codeunits maintain state across invocations within the same session; cross-session truth (active flag, run id, recorded user) lives in `Recording State_TSA_TSL` (table 72930457).
...
- **Field Filtering**: `FilterManager.FilterFields()` reads the selected fields for the table from `Field Select Buffer_TSA_TSL` (72930455) and, if any are selected, keeps only those keys in the JsonObject before storage.
...
- **Custom Filters:** Extend `Table Filter_TSA_TSL` with additional metadata—`Filter Manager` already honors include/exclude records and field lists.
...
1. Open `src/Codeunits/AnalysisEngine.Codeunit.al` (72930454).
...
5. Optionally extend `Analysis Type_TSA_TSL` enum for filtering.

### Scenario 2: Exclude Custom Tables from Capture
1. Open Setup_TSA_TSL → click Table Filters.
...
1. Create new page sourcing temporary `Change Buffer_TSA_TSL`.
